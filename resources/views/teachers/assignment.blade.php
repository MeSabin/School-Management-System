...
<div class="tw-rounded-lg tw-mt-10 tw-min-h-40 tw-p-6 tw-bg-white">
    @if(isset($message))
        ...
        <div class="tw-mb-8 tw-p-4 tw-pl-0 tw-border-gray-300 tw-flex tw-justify-between tw-border-b">
            <div class="tw-flex tw-gap-2 tw-flex-col">
                ...
                <div class="tw-font-medium tw-text-sm tw-text-gray-500">Assigned to: 
                    <span class="tw-text-sm tw-font-semibold tw-text-gray-500">{{ $detail['assignment']->group }}</span>
                </div> 
                <div class="tw-text-gray-500 tw-font-semibold tw-inline-block">
                    <a href="{{$detail['assignment']->file_path}}" 
                ...
                <div class="tw-text-gray-500 tw-font-medium tw-text-sm tw-mt-1">Status: 
                    @if (@isset($detail['status']))      
                    @if ($detail['status']=='Available')
                        <label class="tw-text-green-600 tw-px-3 tw-text-base tw-ml-2">{{$detail['status']}}</label>
                    @else
                        <label class="tw-text-red-600 tw-px-3 tw-text-base tw-ml-2">{{$detail['status']}}</label>
                    @endif
                @endif
                </div>
            ...
            <div class="tw-flex tw-gap-10 tw-items-end">
                    <a href="{{route('viewSubmissions', ['group'=>$detail['assignment']->group, 'assignment_id'=>$detail['assignment']->id])}}" class="tw-px-2 tw-py-2 tw-rounded-lg tw-bg-purple-50 tw-text-purple-500 tw-text-sm tw-font-semibold tw-border tw-border-purple-500 tw-duration-300 hover:tw-bg-purple-600 hover:tw-text-white hover:tw-border-purple-700">Submissions</a>
                    <button data-modal-target="popup-modal-{{$detail['assignment']->id}}" data-modal-toggle="popup-modal-{{$detail['assignment']->id}}" class="delete tw-bg-red-50 tw-text-red-600 tw-border tw-border-red-600 tw-px-3 tw-font-semibold tw-text-sm tw-py-2 tw-rounded-lg tw-duration-300 hover:tw-bg-red-500 hover:tw-text-white hover:tw-border-red-700">Remove</button>
